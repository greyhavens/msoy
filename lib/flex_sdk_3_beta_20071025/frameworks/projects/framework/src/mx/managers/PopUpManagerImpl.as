...
public class PopUpManagerImpl implements IPopUpManager
{
    ...
    private function showModalWindow(o:PopUpData):void
    {
        ...
        if (duration)
        {
            ...
            if (blurAmount)
            {
                const blur:Blur = new Blur(Application.application);
    ...
    private function popupRemovedHandler(event:Event):void
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            ...
            {
                const popUpParent:DisplayObject = o.parent,
    ...
    private function effectEndHandler(event:EffectEvent):void
    {
        ...
        {
            const o:PopUpData = popupInfo[i];
            const e:IEffect = event.effectInstance.effect;
...
class PopUpData
{
    ...
    {
        var s:Rectangle;  
        if (DisplayObject(event.target).root is Stage)
        	s = IUIComponent(event.target).systemManager.screen;
        else
        	s = ISystemManager(DisplayObject(event.target).root).screen;
