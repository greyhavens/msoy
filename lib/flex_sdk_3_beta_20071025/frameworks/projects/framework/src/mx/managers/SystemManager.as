						   ...
						   IFlexModuleFactory, ISystemManager
{
	...
	mx_internal static var lastSystemManager:SystemManager;
        ...
        /** Hackery */
        public function get displayListHandle () :DisplayObject
        {
            return _displayListHandle;
        }

        protected var _displayListHandle :DisplayObject;
	...
	public function SystemManager()
	{
		...
		if (!stage) {
			isStageRoot = false;
                        _displayListHandle = this.root;
                } else {
                    _displayListHandle = stage;
                }
		...
		else
		{
			if (value is String)
			{
				if (value.charAt(0) == "#")
	            {
	                ...
	                var n:Number = Number("0x" + value.slice(1));
	...
	private function docFrameHandler(event:Event = null):void
	{
		...
		// TODO!!! Uncomment this once we figure out how to automatically link in NativeDragManagerImpl
		/* if (Capabilities.playerType == "Desktop")
		{
			...
			// from the desktop.
			// TODO!!! Add support to allow developer to opt-out of DragManager support on a per 
			// window basis. This means adding a new flag to the root tags. 	
	...
	private function installCompiledResourceBundles():void
	{
		...
		resourceManager.mx_internal::installCompiledResourceBundles(
