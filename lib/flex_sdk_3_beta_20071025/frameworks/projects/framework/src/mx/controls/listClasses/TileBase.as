...
//							delete rowMap[oldItem.name];
											  ...
											  scrollUp:Boolean):void
    {
		...
		// TODO probably should be calling addToFreeItemRenderers here
											    ...
											    scrollUp:Boolean):void
    {
		...
		else
		{
			...
			moveBlockDistance = modDeltaPos * columnWidth;
			if (modDeltaPos)
				curX = listItems[0][modDeltaPos-1].x + listItems[0][modDeltaPos-1].width;
	...
	override protected function scrollHandler(event:Event):void
	{
		...
		if (event is ScrollEvent)
		{
			...
			if (scrollBar == verticalScrollBar)
			{
				...
				if (Math.abs(delta) >= listItems.length)
				{
			...
			else
			{
				...
				if (Math.abs(delta) >= listItems[0].length)
				{
	...
	override public function scrollToIndex(index:int):Boolean
	{
		...
		var numItemsVisible:int = (listItems.length * listItems[0].length);
