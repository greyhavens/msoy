...
public class ComboBoxAutomationImpl extends ComboBaseAutomationImpl
{
    ...
    public function ComboBoxAutomationImpl(obj:ComboBox)
    {
        ...
        //need to up the priority on this, cause comboBox swallows all the keys...
        obj.addEventListener(KeyboardEvent.KEY_DOWN, 
                            comboBoxKeyDownHandler, false, EventPriority.DEFAULT+1, true);
    }
     ...
     * @private
     */    
    protected function comboBoxKeyDownHandler(event:KeyboardEvent):void
    {
        ...
        if(event.target == comboBox && 
            ...
            event.keyCode != Keyboard.ENTER)// handled by UIComponent
            recordAutomatableEvent(event);
    }
    ...
    */
    override protected function setupEditHandler():void
    {
        ...
        // If it is non-editable we setup a listener for the comboBox.
        // We donot keep both at the same time to avoid double recording.
        if(comboBox.editable)
        {
            text.addEventListener(KeyboardEvent.KEY_DOWN, 
                            textKeyDownHandler, false, EventPriority.DEFAULT+1);
            comboBox.removeEventListener(KeyboardEvent.KEY_DOWN, 
                        comboBoxKeyDownHandler, false);
        }
        else
        {
            ...
            comboBox.addEventListener(KeyboardEvent.KEY_DOWN, 
                        comboBoxKeyDownHandler, false, EventPriority.DEFAULT+1);
        }
    }
