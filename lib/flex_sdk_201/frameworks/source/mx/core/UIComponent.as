                         ...
                         IValidatorListener
{
    ...
    public function UIComponent()
    {
        ...
        tabEnabled = (this is IFocusManagerComponent); // We are tab enabled by default if IFocusManagerComponent
        ...
        // is complete. It will be set visible when the 'initialized' flag is set.
        $visible = false;
     ...
     *  even though we didn't expect it.  That causes us to do a removeEventListener
     *  twice which messes up some internal thing in the player so that
    ...
    public function set nestLevel(value:int):void
    {
        ...
        if (value > 1 && _nestLevel != value)
        {
            ...
            for (var i:int = 0; i < n; i++)
            {
                ...
                {
                    var textField:UITextField = childList.getChildAt(i) as UITextField;
    ...
    public function set document(value:Object):void
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            ...
            if (child.document == _document ||
                child.document == Application.application)
            {
     ...
     *  the styleName. For example, if you have a class style named <code>".bigText"</code>,
     *  set the styleName property to <code>"bigText"</code> (no period).</p>
     *
     *  <p>If this is an IStyleClient (typically a UIComponent), all styles in the
     *  styleName ojbect are used by this component.</p>
     *
     *  @default null
     */
    ...
    mx_internal function updateCallbacks():void
    {
        ...
        if (systemManager)
        {
            ...
            {
                _systemManager.displayListHandle.addEventListener(Event.RENDER, callLaterDispatcher);
                _systemManager.displayListHandle.addEventListener(Event.ENTER_FRAME, callLaterDispatcher);
     ...
     *
     *  <p>Before each update of the screen, the Flash Player calls
                              ...
                              args:Array /* of Object */ = null):void
    {
        ...
        if (sm && sm.stage)
        {
            ...
            {
                // trace("  added");
                sm.displayListHandle.addEventListener(Event.RENDER, callLaterDispatcher);
                sm.displayListHandle.addEventListener(Event.ENTER_FRAME, callLaterDispatcher);
                listeningForRender = true;
            }
    ...
    mx_internal function cancelAllCallLaters():void
    {
        ...
        if (sm && sm.stage)
        {
            if (listeningForRender)
            {
                sm.displayListHandle.removeEventListener(Event.RENDER, callLaterDispatcher);
                sm.displayListHandle.removeEventListener(Event.ENTER_FRAME, callLaterDispatcher);
     ...
     *  is to set the default size to the current size of the component.</p>
     */
    protected function measure():void
    {
        // can get NaN if percentages are used
        measuredWidth = !isNaN(_width) ? _width : 0;
        measuredHeight = !isNaN(_height) ? _height : 0;
    }
    ...
    public function setFocus():void
    {
        ...
        if (sm && sm.stage)
        {
            ...
            else
            {
                ...
                sm.displayListHandle.addEventListener(Event.ENTER_FRAME, setFocusLater);
            }
        }
        else
        {
    ...
    {
        var transition:Effect = playStateTransition ?
                            getTransition(_currentState, requestedCurrentState) : null;
    ...
    private function getState(stateName:String):State
    {
        ...
        throw new ArgumentError(StringUtil.substitute(resourceStateUndefined, stateName));
    ...
    private function findCommonBaseState(state1:String, state2:String):String
    {
        ...
        // Quick exit if either state is not basedOn another state
     ...
     */
    private function getTransition(oldState:String, newState:String):Effect
    {
        var result:Effect = null;       // Current candidate
     ...
     *  or setting selector styles unnecessarily.</p>
     *
    ...
    public function regenerateStyleCache(recursive:Boolean):void
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            ...
            else if (child is UITextField)
            {
                // Does this object already have a proto chain? 
                // If not, there's no need to regenerate a new one.
                if (UITextField(child).inheritingStyles)
                    StyleProtoChain.initTextField(UITextField(child));
            }
        }
    }
     ...
     *
     *  <p>Calling the setStyle() method can result in decreased performance.
    ...
    mx_internal function initThemeColor():Boolean
    {
        ...
        if (!tc && styleName && !(styleName is ISimpleStyleClient))
        {
    ...
    public function determineTextFormatFromStyles():UITextFormat
    {
        ...
        if (!textFormat)
        {
            ...
            textFormat.font = StringUtil.trimArrayElements(_inheritingStyles.fontFamily,",");
    ...
    private function processEffectFinished(effectInsts:Array):void
    {
        ...
        for (var i:int = _effectsStarted.length - 1; i >= 0; i--)
        {
            ...
            {
                var effectInst:EffectInstance = effectInsts[j];
                if (effectInst == _effectsStarted[i])
                {
                    // Remove the effect from our array.
                    var removedInst:EffectInstance = _effectsStarted[i];
    ...
    private function callLaterDispatcher(event:Event):void
    {
        ...
        else
        {
            ...
            {
                var layoutErrorEvent:DynamicEvent = new DynamicEvent("layoutError");
    ...
    protected function focusInHandler(event:FocusEvent):void
    {
        if (isOurFocus(DisplayObject(event.target)))
        {
            ...
            Container.checkFocus(event.relatedObject, this);    
        }
    }
    ...
    private function addedHandler(event:Event):void
    {
        ...
        if (parent is Container && Container(parent).creatingContentPane)
        {
    ...
    private function setFocusLater(event:Event = null):void
    {
        ...
        if (sm && sm.stage)
        {
            sm.displayListHandle.removeEventListener(Event.ENTER_FRAME, setFocusLater);
            if (UIComponentGlobals.nextFocusObject) {
                try {
