 ...
 *  However, the DropShadowFilter, like all bitmap filters, can be computationally
 *  expensive. If the DropShadowFilter is applied to a
 *  DisplayObject, then the drop shadow is recalculated whenever
 *  the appearance of the object changes. If the DisplayObject
 *  is animated (using a Resize effect, for example), then the
 *  presence of drop shadows hurts the animation refresh rate.
 *
 *  <p>This class optimizes drop shadows for a common case. If you
 *  are applying a drop shadow to a rectangularly-shaped object
 *  whose edges fall on pixel boundaries, then this class should
 *  be used instead of using the DropShadowFilter directly.</p>
 *
 *  <p>This class accepts the first four parameters that
 *  are passed to DropShadowFilter: <code>alpha</code>, <code>angle</code>, <code>color</code>, and 
 *  <code>distance</code>. In addition, this class accepts the corner radii
 *  for each of the four corners of the rectangularly-shaped object
 *  that is casting a shadow.</p>
 *
 *  <p>Once those 8 values have been set, this class pre-computes the
 *  drop shadow in an offscreen Bitmap. When the <code>drawShadow()</code>
 *  method is called, pieces of the precomputed drop shadow are 
 *  copied onto the passed-in Graphics object.</p>
 *  
...
public class RectangularDropShadow
{
	...
	/**
	 *  @private
	 *  Remember whether any of the public properties have changed since
	 *  the most recent call to drawDropShadow.
	 */
     ...
     *  
     *  @param x The horizontal offset of the drop shadow, based on the Graphics object's position.
     *  
     *  @param y The vertical offset of the drop shadow, based on the Graphics object's position.
     *  
    ...
    */
	...
	{	
		// If any parameters of the shadow have changed, then regenerate the
		// offscreen bitmaps
		if (changed)
		{
			createShadowBitmaps();
		...
		{
			var tlWidth:Number = Math.min(tlRadius + widthThickness, maxCornerWidth);
			var tlHeight:Number = Math.min(tlRadius + heightThickness, maxCornerHeight);
		...
		{
			var trWidth:Number = Math.min(trRadius + widthThickness, maxCornerWidth);
			var trHeight:Number = Math.min(trRadius + heightThickness, maxCornerHeight);
		...
		{
			var blWidth:Number = Math.min(blRadius + widthThickness, maxCornerWidth);
			var blHeight:Number = Math.min(blRadius + heightThickness, maxCornerHeight);
		...
		{
			var brWidth:Number = Math.min(brRadius + widthThickness, maxCornerWidth);
			var brHeight:Number = Math.min(brRadius + heightThickness, maxCornerHeight);
		...
		{		
			...
			g.drawRect(x - leftThickness, 
					   ...
					   leftThickness,
					   height + topThickness + bottomThickness - tlHeight - blHeight);
			g.endFill();
		}
		...
		{		
			...
			g.drawRect(x + width,
					   ...
					   rightThickness,
					   height + topThickness + bottomThickness - trHeight - brHeight);
			g.endFill();
		}
		...
		{		
			...
			g.drawRect(x - leftThickness + tlWidth,
					   y - topThickness, 
					   width + leftThickness + rightThickness - tlWidth - trWidth,
					   topThickness);
			g.endFill();
		}
		...
		if (bottomShadow)
		{
			...
			g.drawRect(x - leftThickness + blWidth,
					   y + height,
					   width + leftThickness + rightThickness - blWidth - brWidth, 
					   bottomThickness);
			g.endFill();		
		}
	}	
	...
	/**
	 *  @private
	 *  Render the drop shadow for the rounded rectangle in a small BitmapData
	 *  object.  The shadow will be copied onto the graphics object passed into
	 *  drawDropShadow.
	 */
	...
	{		
		...
		GraphicsUtil.drawRoundRectComplex(g, 0, 0, roundRectWidth, roundRectHeight,
