 ...
 *  You typically use SWFLoader for loading a Flex 2 application
 *  into a host Flex 2 application.
 *
 *  <p><strong>Note:</strong> You can also use the SWFLoader control to load
 *  a GIF, JPEG, or PNG image file at runtime, or embed any of these files 
...
public class SWFLoader extends UIComponent
{
	 ...
	 *  @private
     */
	private static var resourceOutOfRange:String;
    
	/**
	 *  @private
     */
	private static var resourceNotLoadable:String;
	...
	{
		resourceOutOfRange = packageResources.getString("outOfRange");

		resourceNotLoadable = packageResources.getString("notLoadable");
	}
	...
	/**
	 *  @private
	 *  Should also be [Exclude]d if we implement it.
	 */
	override public function get baselinePosition():Number
	{
		// We don't know the baseline position of the content.
		return NaN;
	}
	 ...
	 *  Window.loadContent() and RectangularBorder.updateDisplayList()
	...
	private function loadContent(classOrString:Object):DisplayObject
	{
		...
		else if (url)
		{
			...
			if ( (Capabilities.isDebugger == true) && 
				 (url.indexOf(".jpg") == -1) && 
				 (Application.application.systemManager.loaderInfo.url.indexOf("debug=true") > -1) )
		...
		{
			throw new Error(StringUtil.substitute(resourceNotLoadable, source));
		}
