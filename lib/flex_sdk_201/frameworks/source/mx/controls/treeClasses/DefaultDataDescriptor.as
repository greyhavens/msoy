 ...
 */
public class DefaultDataDescriptor implements ITreeDataDescriptor, IMenuDataDescriptor
{
     ...
     *  @private
     */
	private var XMLListCollectionCache:Dictionary = new Dictionary(true);
	...
	public function getChildren(node:Object, model:Object=null):ICollectionView
	{
		...
		{
			var oldCollection:XMLListCollection = XMLListCollectionCache[node];
			if (!oldCollection)
			{
				childrenCollection =  new XMLListCollection(children);
				XMLListCollectionCache[node] = childrenCollection;
			}
			else
			{
				childrenCollection = oldCollection;
				
				//We want to set the source of the children collection only if necessary
				if (XMLList(XMLListCollection(childrenCollection).source) != XMLList(children))
	...
	public function addChildAt(parent:Object, newChild:Object, index:int, model:Object=null):Boolean
	{
		...
		{
			var children:IList = IList(getChildren(parent, model));
			if (!children)
			{
				if (parent is XML)
				{
