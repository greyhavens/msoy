...
public class ScrollBar extends UIComponent
{
    ...
    public function set scrollPosition(value:Number):void
    {
        ...
        if (scrollThumb)
        {
            ...
            if (!isScrolling)
            {
                ...
                scrollThumb.move(0, Math.round(y));
            }
        }
    }
    ...
    override protected function createChildren():void
    {
        ...
        if (!scrollTrack)
        {
            var scrollTrackClass:Class = getStyle("trackSkin");
            scrollTrack = new scrollTrackClass();
                                                  ...
                                                  unscaledHeight:Number):void
    {
        ...
        scrollTrack.y = 0;
        scrollTrack.height = virtualHeight;

        downArrow.move(
            0, virtualHeight - downArrow.getExplicitOrMeasuredHeight());
    ...
    private function scrollTrack_mouseDownHandler(event:MouseEvent):void
    {
        ...
        systemManager.displayListHandle.addEventListener(MouseEvent.MOUSE_MOVE, 
                            stage_scrollTrack_mouseMoveHandler);
        // in case we go offscreen
        systemManager.displayListHandle.addEventListener(Event.MOUSE_LEAVE, 
                            scrollTrack_mouseLeaveHandler);
        
        trackPosition = event.localY;

        if (isNaN(oldPosition))
            oldPosition = scrollPosition;

        trackScrollRepeatDirection =
            scrollThumb.y + scrollThumb.height < event.localY ? 1 :
            scrollThumb.y > event.localY ? -1 : 0;
    ...
    private function scrollTrack_mouseLeaveHandler(event:Event):void
    {
        ...
        systemManager.removeEventListener(
            MouseEvent.MOUSE_MOVE, scrollTrack_mouseMoveHandler, true);
        // in case we go offscreen
        systemManager.displayListHandle.removeEventListener(MouseEvent.MOUSE_MOVE, 
                            stage_scrollTrack_mouseMoveHandler);
        // in case we go offscreen
        systemManager.displayListHandle.removeEventListener(Event.MOUSE_LEAVE, 
                            scrollTrack_mouseLeaveHandler);
