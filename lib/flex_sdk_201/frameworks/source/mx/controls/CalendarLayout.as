							...
							implements IFocusManagerComponent
{
     ...
     *  @private
     */
	private var todaysLabelReference:UITextField = null;
    ...
    [Inspectable(category="Other", arrayType="String", defaultValue="S,M,T,W,T,F,S")]
    ...
    */
    ...
    override protected function createChildren():void
    {
        ...
        for (var columnIndex:int = 0; columnIndex < 7; columnIndex++)
        {
            ...
            for (var rowIndex:int = 0; rowIndex < 7; rowIndex++)
            {
				...
				dayBlocksArray[columnIndex][rowIndex] = new UITextField();
        ...
        if (!todayIndicator)
		{
			...
			if (todayIndicator is DateChooserIndicator)
				DateChooserIndicator(todayIndicator).indicatorColor = "todayColor";
			if (todayIndicator is ISimpleStyleClient)
				ISimpleStyleClient(todayIndicator).styleName = this;
			addChildAt(DisplayObject(todayIndicator), 0);
		...
		if (!rollOverIndicator)
		{
			...
			if (rollOverIndicator is DateChooserIndicator)
				DateChooserIndicator(rollOverIndicator).indicatorColor = "rollOverColor";
	...
	override protected function measure():void
    {
        ...
        // Avoid calling getter in loop which gets dayNames from ResourceBundle
        var tempDayNames:Array = dayNames;
        for (var dayOfWeek:int = 0; dayOfWeek < 7; dayOfWeek++)
        {
            lineMetrics = measureText(tempDayNames[dayOfWeek]);
												  ...
												  unscaledHeight:Number):void
    {
        ...
        for (var columnIndex:int = 0; columnIndex < 7; columnIndex++)
        {
            ...
            for (var rowIndex:int = 0; rowIndex < 7; rowIndex++)
            {
                var label:UITextField = dayBlocksArray[columnIndex][rowIndex];
                if (rowIndex==0)
    ...
    private function drawDayNames():void
    {
        ...
        for (var columnIndex:int = 0; columnIndex < 7; columnIndex++)
        {
            ...
            dayBlocksArray[columnIndex][0].text = tempDayNames[dayOfWeek];
        }
    }
	 ...
	 *  @private
	 */
    private function setSelectedMonthAndYear(monthVal:int = -1, yearVal:int = -1):void
    {
        ...
        for (dayNumber = 1; dayNumber <= daysInMonth; dayNumber++)
        {
            ...
            var todayLabel:UITextField = dayBlocksArray[columnIndex][rowIndex];//this["dayBlock" + columnIndex+"label" + rowIndex];
            ...
            if (_selectableRange)
            {
                var t:int = selRangeMode;
                switch (t)
                {
                    case 1:
                    {
                        if (cellDate < _selectableRange.rangeStart ||
							cellDate >_selectableRange.rangeEnd)
                        {
                    ...
                    case 3:
                    {
                        if (cellDate > _selectableRange.rangeEnd)
                        {
                            todayLabel.enabled = false;
                            disabledArrays[columnIndex][rowIndex] = true;
                        }
                    ...
                    case 4:
                    {
                        ...
                        var selRangeString:String = "" +
							_selectableRange.getFullYear() + "." +
							_selectableRange.getMonth() + "." +
							_selectableRange.getDate();
            ...
            if (_disabledRanges.length>0)
            {
                ...
                {
                    switch (disabledRangeMode[dRanges])
                    {
                        case 1:
                        {
                            if (cellDate >= _disabledRanges[dRanges].rangeStart && cellDate <= _disabledRanges[dRanges].rangeEnd)
                            {
                                todayLabel.enabled = false;
                                disabledArrays[columnIndex][rowIndex] = true;
                            }
                        ...
                        case 2:
                        {
                            if (cellDate >= _disabledRanges[dRanges].rangeStart)
                            {
                                todayLabel.enabled = false;
                                disabledArrays[columnIndex][rowIndex] = true;
                            }
                        ...
                        case 3:
                        {
                            if (cellDate <= _disabledRanges[dRanges].rangeEnd)
                            {
                                todayLabel.enabled = false;
                                disabledArrays[columnIndex][rowIndex] = true;
                            }
                        ...
                        case 4:
                        {
                            ...
                            var disRangeString:String = "" +
								_disabledRanges[dRanges].getFullYear() + "." +
								_disabledRanges[dRanges].getMonth() + "." +
								_disabledRanges[dRanges].getDate();
	 ...
	 *  @private
	 */
    private function dispatchChangeEvent(triggerEvent:Event = null):void
    {
	 ...
	 */
    private function checkDateIsDisabled(value:Date):Boolean
    {
        if (!value)
            return false;
        ...
        if (_selectableRange)
        {
            switch (selRangeMode)
            {
                case 1:
                {
                    if (value < _selectableRange.rangeStart ||
						value > _selectableRange.rangeEnd)
                    {
                ...
                case 3:
                {
                    if (value > _selectableRange.rangeEnd)
                        selectedDateIsDisabled = true;
                    break;
                }
                ...
                case 4:
                {
                    if (value > _selectableRange || value < _selectableRange)
                        selectedDateIsDisabled = true;
                    break;
                }
        ...
        if (_disabledRanges.length > 0)
        {
            for (var dRanges:int = 0; dRanges < _disabledRanges.length; dRanges++)
            {
                switch (disabledRangeMode[dRanges])
                {
                    case 1:
                    {
                        if (value >= _disabledRanges[dRanges].rangeStart &&
							value <= _disabledRanges[dRanges].rangeEnd)
						{
                            selectedDateIsDisabled = true;
						}
                        break;
                    ...
                    case 3:
                    {
                        if (value <= _disabledRanges[dRanges].rangeEnd)
                            selectedDateIsDisabled = true;
                        break;
                    }
	 ...
	 *  @private
	 */
    private function checkMonthValidity(value:int):Boolean
    {
        var invalidMonth:Boolean = false;
		...
		if (_selectableRange)
        {
            switch (selRangeMode)
            {
                case 1:
                {
                    if (value < _selectableRange.rangeStart.getMonth() ||
						value > _selectableRange.rangeEnd.getMonth())
                    {
                        invalidMonth = true;
                    }
                    break;
                ...
                case 3:
                {
                    if (value > _selectableRange.rangeEnd.getMonth())
                        invalidMonth = true;
                    break;
                }
                ...
                case 4:
                {
                    if (value > _selectableRange.getMonth() ||
						value < _selectableRange.getMonth())
                    {
                        invalidMonth = true;
                    }
                    break;
	 ...
	 *  @private
	 */
    private function checkYearValidity(value:int):Boolean
    {
        var invalidYear:Boolean = false;
        ...
        if (_selectableRange)
        {
            switch (selRangeMode)
            {
                case 1:
                {
                    if (value < _selectableRange.rangeStart.getFullYear() ||
						value > _selectableRange.rangeEnd.getFullYear())
                    {
                        invalidYear = true;
                    }
                    break;
                ...
                case 3:
                {
                    if (value > _selectableRange.rangeEnd.getFullYear())
                        invalidYear = true;
                    break;
                }
                ...
                case 4:
                {
                    if (value > _selectableRange.getFullYear() ||
						value < _selectableRange.getFullYear())
                    {
                        invalidYear = true;
                    }
                    break;
	 ...
	 */
    private function addToSelected(newDate:Date, range:Boolean = false):void
    {
     ...
     */
	private function incrementDate(value:Date, amount:int = 1):Date
	{
     ...
     */
	private function isSelected(newDate:Date):Boolean
	{
     ...
     */
	private function removeRangeFromSelection(startDate:Date, endDate:Date):void
    {
     ...
     */
	private function setSelectedIndicators():void
	{
     ...
     *  @private
     */
    private function addSelectionIndicator(columnIndex:int, rowIndex:int):void
    {
        if (!selectionIndicator[columnIndex][rowIndex])
        {
			...
			if (selectionIndicator[columnIndex][rowIndex] is DateChooserIndicator)
				DateChooserIndicator(selectionIndicator[columnIndex][rowIndex]).indicatorColor =
					"selectionColor";
			...
			var selCell:UITextField = dayBlocksArray[columnIndex][rowIndex];
     ...
     *  @private
     */
	private function removeSelectionIndicator(columnIndex:int,
											 rowIndex:int):void
	{
		if (selectionIndicator[columnIndex][rowIndex])
		{
    ...
    private function mouseMoveHandler(event:MouseEvent):void
    {
        ...
        var selCell:UITextField = dayBlocksArray[colIndex][rowIndex];
    ...
    private function mouseUpHandler(event:MouseEvent):void
    {
        ...
        var colIndex:int = Math.floor((mouseX-paddingLeft) / cellWidth);
        var selCell:UITextField = dayBlocksArray[colIndex][rowIndex];//this["dayBlock"+colIndex+"label"+rowIndex];
        // If it is disabled, we're done.
