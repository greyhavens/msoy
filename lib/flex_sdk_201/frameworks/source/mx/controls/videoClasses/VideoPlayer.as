...
public class VideoPlayer extends Video
{
	 ...
	 *  @private
     */	
	private static var resourceNullURL:String;

    /**
	 *  @private
     */	
	private static var resourceBadArgs:String;
	...
	{
		resourceNullURL = packageResources.getString("nullURL");
		resourceBadArgs = packageResources.getString("badArgs");
	}
    ...
    {
        if (readyDispatched)
            internalVideoWidth = super.videoWidth;
    ...
    {
        if (readyDispatched)
            internalVideoHeight = super.videoHeight;
    ...
    {
        if (url == null)
            throw new ArgumentError(resourceNullURL);
    ...
    public function doUpdateProgress(event:Event):void 
    {
        ...
        if (ns.bytesTotal >= 0 && ns.bytesLoaded != _priorBytesLoaded)
            dispatchEvent(new ProgressEvent(ProgressEvent.PROGRESS, true, false,
    ...
    public function onMetaData(info:Object):void 
    {
		...
		var metadataEvent:MetadataEvent =
			new MetadataEvent(MetadataEvent.METADATA);
    ...
    private function _play(... rest):void 
    {
        ...
        switch (rest.length) 
		{
			...
			case 2:
				...
				break;
			default:
				throw new ArgumentError(resourceBadArgs);
        }
    }
