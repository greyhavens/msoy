 ...
 *
 *  @default NumericStepperDownSkin
 */
							...
							IListItemRenderer
{
    ...
    {
        return inputField ? inputField.tabIndex : -1;
    }
    ...
    {
        if (stage)
            stage.focus = inputField.getTextField();
    }
    ...
    private function inputField_keyDownHandler(event:KeyboardEvent):void
    {
        ...
        switch (event.keyCode)
        {
			...
			case Keyboard.TAB:
            {
                var inputValue:Number = Number(inputField.text);
                if (inputValue != value &&
					(Math.abs(inputValue - value) >= 0.000001 ||
					 isNaN(inputValue)))
                {
                    var newValue:Number = checkValidValue(Number(inputField.text));
                    inputField.text = newValue.toString();
