...
public class List extends ListBase implements IIMESupport
{
     ...
     *  If there are any items in the list, the baseline position is
     *  computed based on the baseline position of item renderer.
     *  The baseline position is the y-coordinate of the first line of text 
     *  of the component. 
     */
    ...
    private function adjustVerticalScrollPositionDownward(rowCount:int):Boolean
    {
        ...
        var item:IListItemRenderer = getMeasuringRenderer();
        item.explicitWidth = listContent.width;
        ...
        if (event is ScrollEvent)
        {
            ...
            if (scrollBar == verticalScrollBar)
            {
                ...
                if (Math.abs(delta) >= listItems.length - lockedRowCount)
                {
                    ...
                    {
                        lastSeekPending = new ListBaseSeekPending(CursorBookmark.CURRENT, delta)
                        e.addResponder(new ItemResponder(seekPendingResultHandler, seekPendingFailureHandler,
                                                        lastSeekPending));
    ...
    mx_internal function setupRendererFromData(item:IListItemRenderer, data:Object):void
    {
        ...
        {
            if (data != null)
                IDropInListItemRenderer(item).listData = makeListData(data, itemToUID(data), 0);
            else
                IDropInListItemRenderer(item).listData = null;
        }
    ...
    public function createItemEditor(colIndex:int, rowIndex:int):void
    {
        ...
        // use weak reference
        systemManager.displayListHandle.addEventListener(MouseEvent.MOUSE_DOWN, editorMouseDownHandler, true, 0, true);
    ...
    public function destroyItemEditor():void
    {
        ...
        if (itemEditorInstance)
        {
            ...
            systemManager.displayListHandle.removeEventListener(MouseEvent.MOUSE_DOWN, editorMouseDownHandler, true);
    ...
    private function itemEditorItemEditBeginHandler(event:ListEvent):void
    {
        ...
        systemManager.displayListHandle.addEventListener(Event.DEACTIVATE, deactivateHandler, false, 0, true);
            ...
            if (!bFieldChanged)
            {
                ...
                if (bChanged)
                {
                    if (collection is IList)
                        IList(collection).setItemAt(data, event.rowIndex);
                    else
