...
/**
 *  Vertical alignment of the tabs.
 ...
 public class ButtonBar extends NavBar implements IFocusManagerComponent
{
                                                  ...
                                                  unscaledHeight:Number):void
    {
        ...
        var w:Number = unscaledWidth - vm.left - vm.right -
                       (isHorizontal ? horizontalGap * (n - 1) : 0);
        var h:Number = unscaledHeight - vm.top - vm.bottom -
                       (isVertical ? verticalGap * (n - 1) : 0);
        if (!w || !h)
            return;
        ...
        if (recalcButtonHeights)
        {
            ...
            for (i = 0; i < n; i++)
            {
                ...
                if (isNaN(c.explicitHeight))
                {
                    ...
                    {
                        c.minHeight = c.maxHeight = buttonHeight;
