 ...
 * 
 *  For the CheckBox 
 *  and RadioButton controls, this event is dispatched when the <code>selected</code> 
 *  property changes.
 *
 ...
 *    selectedDownIcon="<i>No default</i>"
       ...
       IFocusManagerComponent, IListItemRenderer
{
    ...
    override protected function measure():void
    {
        ...
        if (label)
        {
            ...
            textWidth += (getStyle("paddingLeft") +
                          ...
                          getStyle("textIndent"));
            textHeight += (getStyle("paddingTop") +
    ...
    mx_internal function viewSkin():void
    {
        ...
        if (!enabled)
            skinName = selected ? selectedDisabledSkinName : disabledSkinName;

        else if (phase == ButtonPhase.UP)
            skinName = selected ? selectedUpSkinName : upSkinName;

        else if (phase == ButtonPhase.OVER)
            skinName = selected ? selectedOverSkinName : overSkinName;

        else if (phase == ButtonPhase.DOWN)
            skinName = selected ? selectedDownSkinName : downSkinName;

        viewSkinForPhase(skinName);
    }
    ...
    mx_internal function viewSkinForPhase(skinName:String):void
    {
        ...
        if (!newSkin)
        {
            ...
            if (newSkinClass)
            {
                ...
                {
                    ProgrammaticSkin(newSkin).validateDisplayList()
                }
     ...
     */
    mx_internal function viewIconForPhase(tempIconName:String):void
    {
        // Has this icon already been created?
        var newIcon:IFlexDisplayObject =
            IFlexDisplayObject(getChildByName(tempIconName));
     ...
     *  Notice that layout hinges on a variable, centerContent, which is
     *  set to true in Button, but false in CheckBox and RadioButton.
     *  When centerContent is false, the text/icon are always left-aligned.
     *  When centerContent is true, the text/icon are aligned
     *  based on the textAlign style setting.
     */
    ...
    mx_internal function buttonReleased():void
    {
        ...
        systemManager.displayListHandle.removeEventListener(
    ...
    */
    ...
    protected function mouseDownHandler(event:MouseEvent):void
    {
        ...
        systemManager.addEventListener(
            MouseEvent.MOUSE_UP, systemManager_mouseUpHandler, true);
        systemManager.displayListHandle.addEventListener(
            Event.MOUSE_LEAVE, stage_mouseLeaveHandler);
