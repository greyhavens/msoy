...
public class Validator extends EventDispatcher implements IMXMLObject
{
     ...
     *  @private    
     */ 
    private static var resourceRequired:Boolean;

    /**
     *  @private    
     */ 
    private static var resourceRequiredFieldError:String;

    /**
     *  @private    
     */ 
    private static var resourceFieldNotFound:String;    

    /**
     *  @private    
     */ 
    private static var resourceSAttribute:String;

    /**
     *  @private    
     */ 
    private static var resourceSAttributeMissing:String;

    /**
     *  @private    
     */ 
    private static var resourcePAttributeMissing:String;
    ...
    {
        resourceRequired = packageResources.getBoolean("required");
        
        resourceRequiredFieldError =
            packageResources.getString("requiredFieldError");

        resourceFieldNotFound = packageResources.getString("fieldNotFound");

        resourceSAttribute = packageResources.getString("SAttribute");

        resourceSAttributeMissing =
            packageResources.getString("SAttributeMissing");

        resourcePAttributeMissing =
            packageResources.getString("PAttributeMissing");            
    }
                                                 ...
                                                 value:String):Object
    {
        ...
        for (var i:int = 0; i < n; i++)
        {
            ...
            catch(error:Error)
            {
                ...
                {
                    throw new Error(StringUtil.substitute(resourceFieldNotFound, value));
                }
                else
                {                    
                    throw error;
    ...
    [Inspectable(category="Errors", defaultValue="This field is required.")]
    ...
    public function set source(value:Object):void
    {
        ...
        if (value is String)
            throw new Error(StringUtil.substitute(resourceSAttribute, value));
    ...
    private function bundleChanged():void
    {
        ...
        requiredFieldError = resourceRequiredFieldError;
    }
    ...
    /**
     *  Returns <code>true</code> if <code>value</code> is null. 
     * 
     *  @param value The value to test.
     *
     *  @return <code>true</code> if <code>value</code> is null.
     */
    ...
    protected function getValueFromSource():Object
    {
        if (_source && _property)
            ...
            throw new Error(resourceSAttributeMissing);

        else if (_source && !_property)
            throw new Error(resourcePAttributeMissing);
    ...
    private function validateRequired(value:Object):ValidationResult
    {
        ...
        if (validator.required)
        {
            ...
            if (val.length == 0)
            {
                validator.validationError("requiredField",
                    validator.requiredFieldError, baseField);
            }
        }
        */
