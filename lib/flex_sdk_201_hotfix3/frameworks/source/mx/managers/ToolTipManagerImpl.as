             ...
             implements IToolTipManager
{
    ...
    public function registerToolTip(target:DisplayObject,
                                                toolTip:String):void
    {
        var oldToolTip:String;

        if (target is IToolTipManagerClient)
            oldToolTip = IToolTipManagerClient(target).toolTip;
    ...
    {
        var oldErrorString:String = IValidatorListener(target).errorString;
        ...
        else if (oldErrorString && !errorString)
        {
            target.removeEventListener(MouseEvent.MOUSE_OVER,
                                       errorTipMouseOverHandler);
            target.removeEventListener(MouseEvent.MOUSE_OUT,
    ...
    private function positionTip():void
    {
        ...
        else
        {
            ...
            x = Application.application.mouseX + 11;
            y = Application.application.mouseY + 22;
     ...
     *  with an errorString.
     */
    private function errorTipMouseOverHandler(event:MouseEvent):void
    {
        checkIfTargetChanged(DisplayObject(event.target));
    }
     ...
     *  with an errorString.
     */
    private function errorTipMouseOutHandler(event:MouseEvent):void
    {
        checkIfTargetChanged(event.relatedObject);
    }
