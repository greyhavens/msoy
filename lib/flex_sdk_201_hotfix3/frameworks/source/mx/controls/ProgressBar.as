 ...
 *
 *  @default ProgressIndeterminateSkin
 */
...
public class ProgressBar extends UIComponent
{
     ...
     *  @private
     */
    private static var resourceLabel:String;
    ...
    {
        resourceLabel = packageResources.getString("label");
    }
     ...
     *  @private
     */
    private var _indeterminateBar:IFlexDisplayObject;

    /**
     *  @private
     */
    private var _determinateBar:IFlexDisplayObject;

    /**
     *  @private
     */
    private var _track:IFlexDisplayObject;

    /**
     *  @private
     */
    private var _barMask:UIComponent;
    ...
    [Inspectable(category="General", defaultValue="LOADING %3%%")]
     ...
     *    The ProgressBar uses these events to update its status.</li>
     *
     *    <li><code>polled</code> The <code>source</code> property must specify
     *    an object that exposes <code>getBytesLoaded()</code> and
    ...
    override protected function createChildren():void
    {
        ...
        if (!_barMask)
        {
            ...
            UIComponent(_bar).mask = _barMask;
        }
    ...
    override protected function commitProperties():void
    {
        ...
        if (modeChanged)
        {
            ...
            if (_mode == ProgressBarMode.POLLED)
            {
                pollTimer.addEventListener(TimerEvent.TIMER, updatePolledHandler);
                pollTimer.start();
            }
                                                  ...
                                                  unscaledHeight:Number):void
    {
        ...
        else
        {
            ...
            //trace("draw bar.w",Math.round(_track.width),"new w",w ,"%",Math.round(percentComplete),"x",_determinateBar.x);
    ...
    private function layoutContent(newWidth:Number, newHeight:Number):void
    {
        ...
        _indeterminateBar.setActualSize(newWidth + 28, newHeight);
    }
    ...
    private function startPlayingIndeterminate():void
    {
        if (!indeterminatePlaying)
        {
            ...
            pollTimer.addEventListener(TimerEvent.TIMER, updateIndeterminateHandler);
            pollTimer.start();
        }
    }
