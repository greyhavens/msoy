...
{
    /**
     * Adds the secured price to the cache. Subsequent calls to
     * {@link #getSecuredPrice(int, ItemIdent)} for this member/item will return the secured
     * price, unless the cache is full and the secured price is dropped to make room, or the price
     * expires.
     * 
    ...
    /**
     * Gets the secured price for a given member and item. If the member has not previously
     * secured the price, the price is expired, or it is removed from the cache to make room, then
